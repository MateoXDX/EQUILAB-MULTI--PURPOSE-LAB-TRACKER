...
function showConfirmationModal() {
    ...
    $('#rulesModal').removeClass('is-open');
    $('#confirmationModal').addClass('is-open');
}
...
function updateBorrowerFormList() {
    ...
    selectedEquipment.forEach((item, index) => {
        const label = (item.equipment_name || item.equipment_id || "—").toString();
        const row = $("<tr>");
        ...
        input.on("input", function () {
            ...
            typingTimer = setTimeout(() => {
                ...
                if (isNaN(val) || val < 1) {
                    val = 1;
                } else if (val > Number(selectedEquipment[i].available)) {
                    val = Number(selectedEquipment[i].available);
                }
...
function showEquipment() {
    ...
    const bf = document.getElementById("borrowerFormSection");
