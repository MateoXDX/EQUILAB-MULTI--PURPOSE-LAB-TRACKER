...
$(document).ready(function () {
    ...
    $('#equipmentList').on('change', '.equipment-checkbox', function () {
        ...
        if (eq) toggleEquipmentSelection(eq, $(this).is(':checked'));
...
$(document).ready(function () {
    ...
    $('#searchInput').on('input', function () {
        ...
        document.querySelectorAll('#equipmentTable tbody tr').forEach(row => {
            ...
            row.style.display = (id.includes(kw) || name.includes(kw)) ? '' : 'none';
        });
    });
});
...
function populateEquipmentTable(data) {
    const tbody = $('#equipmentList').empty();
    data.forEach(item => {
        ...
        row.append($('<td>').append(
            $(`<input type="checkbox" class="equipment-checkbox" data-id="${item.equipment_id}">`)
                .prop('checked', isChecked).prop('disabled', isDisabled)
                .prop('title', isDisabled ? 'Not Available' : '')
        ));
        row.append(`<td>${item.equipment_id}</td><td>${item.equipment_name}</td><td>${item.available}</td>`);
        tbody.append(row);
    });
}
