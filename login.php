  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .tab-btn.active {
      background: var(--white);
      color: var(--ink);
      box-shadow: 0 1px 6px rgba(15,15,15,0.09);
    }
    ...
    @media (max-width: 860px) {
      ...
      .left-panel { padding: 36px 32px 32px; }
  ...
  <div class="left-panel">
    ...
    <div class="hero-text">
      ...
      <p class="hero-sub">
        ...
        Built for EARIST Bachelor of Science in Applied Physics.
      </p>
    </div>
        ...
        <form id="login-form" autocomplete="off">
          ...
          <button class="btn-primary" type="submit" id="loginBtn">
            <span id="loginBtnInner">
              Sign in
              <svg style="display:inline;vertical-align:middle;margin-left:6px" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
...
<script>
  ...
  function fetchRecentGuests() {
    ...
    .then(data => {
      ...
      const todayCount = data.data.filter(g => g.created_at && g.created_at.startsWith(today)).length;
  ...
  function fetchStats() {
    ...
    fetch('fetch_recent_guests.php').then(r=>r.json()).then(d=>{
      ...
      const today = new Date().toISOString().slice(0,10);
      document.getElementById('statGuests').textContent = d.data.filter(g=>g.created_at&&g.created_at.startsWith(today)).length;
    }).catch(()=>{});
  }
